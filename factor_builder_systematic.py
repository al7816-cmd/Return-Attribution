...
print("%%% FACTOR_BUILDER_SYSTEMATIC SUCCESSFULLY RUNNING %%%")
...
def load_financial_metrics(sp500 = True, starting_date = DEFAULT_STARTING_DATE, tickers = None, db = None):
    ...
    query = "select gvkey, tic, iid, datadate, prccq, cshoq, ceqq, teqq, niq, revtq, oancfy, oibdpq, capxy, ltq, atq, cogsq, xoprq, epspxq, fyearq, fqtr from comp.fundq where gvkey in ("
    ...
    financials['Operating Margin'] = financials['oibdpq'] / financials['revtq']
    financials['Operating Profitability'] = financials['oibdpq'] / financials['ceqq']
    financials['ROE'] = financials['niq'] / financials['ceqq']
    financials = financials.rename(columns={'prccq':'EOQ Closing Price', 'cshoq':'Shares Outstanding','ceqq':'Shareholders Equity Value','niq':'Net Income','revtq':'Revenue','oancfy_q':'Cash Flow From Operations','capxy_q':'CapEx','ltq':'Total Liabilities','atq':'Total Assets','cogsq':'Cost of Goods Sold','xoprq':'Operating Expense', 'oibdpq':'Operating Income'})
...
def build_investment_factor(
    prices: pd.DataFrame,
    fund_raw: pd.DataFrame,
    id_col: str = "gvkey",
    date_col: str = "datadate",
    inv_col: str = "Total Assets",
    long_quantile: float = 0.7,
    short_quantile: float = 0.3,
    eval_window_months: int = 6
):
    """
    Build investment factor using total asset growth with quarterly rebalancing using lagged fundamentals.
    """
    # Daily returns
    ret_daily = prices.pct_change(fill_method=None).dropna(how='all')
    
    # Prepare fundamentals
    fund = fund_raw[[id_col, date_col, inv_col]].copy()
    fund[date_col] = pd.to_datetime(fund[date_col])
    fund[inv_col] = pd.to_numeric(fund[inv_col], errors="coerce")
    fund = fund.dropna()

    # Pivot to get total assets by ticker and quarter
    inv_q = fund.pivot_table(index=date_col, columns=id_col, values=inv_col, aggfunc="last").sort_index().ffill()
    inv_q = inv_q[inv_q.index.month.isin([3, 6, 9, 12])].dropna(axis=1)

    # total assets on balance sheet --> already ready for YoY calculation
    inv_growth_q = inv_q.pct_change(4).dropna()
    ...
    # transform to weights --> long conservative investment (low asset growth), short aggressive investment (high asset growth)
    ranking_q = inv_growth_q.rank(axis=1, ascending=False) # weights constructed to: LONG 500, 499, 498..., SHORT 1, 2, 3, ...
...
def build_profitability_factor(
    prices: pd.DataFrame,
    fund_raw: pd.DataFrame,
    id_col: str = "gvkey",
    date_col: str = "datadate",
    val_col: str = "Operating Profitability",
    long_quantile: float = 0.7,
    short_quantile: float = 0.3,
    eval_window_months: int = 6
):
...
def build_defensive_factor(
    ...
    # Compute cross-sectional volatility signal at each rebalance date
    vol_signal_list = []
    for d in rebal_dates:
        start = d - pd.DateOffset(months=lookback_months)
        window = ret_daily.loc[start:d]
        if window.shape[0] < 2:
            vol_signal_list.append(pd.Series(dtype=float, name=d))
            continue
        sig = window.std()
        sig.name = d
        vol_signal_list.append(sig)

    vol_signal = pd.DataFrame(vol_signal_list).reindex(index=rebal_dates)

    # Low vol = high rank = long, high vol = low rank = short
    ranking_m = vol_signal.rank(axis=1, ascending=False)

    n = ranking_m.shape[1]
    q = int(n * short_quantile)

    weights_m = (
        (ranking_m >= n - q + 1).astype(float) / q
        - (ranking_m <= q).astype(float) / q
    )
...
def save_factors():
    momentum = build_momentum_factor(prices=PRICES)[0].to_frame('momentum')
    ...
    profitability = build_profitability_factor(prices=PRICES, fund_raw=RAW_DATA)[0].to_frame('profitability')
    investment = build_investment_factor(prices=PRICES, fund_raw=RAW_DATA)[0].to_frame('investment')
    defensive = build_defensive_factor(prices=PRICES)[0].to_frame('defensive')
    quality = build_quality_factor(prices=PRICES, fund_raw=RAW_DATA)[0].to_frame('quality')
    ...
    factors = momentum.merge(growth, how='inner', left_on='datadate', right_on='datadate') # GROWTH & MOMENTUM
    factors = factors.merge(value, how='inner', left_on='datadate', right_on='datadate') # VALUE
    factors = factors.merge(profitability, how='inner', left_on='datadate', right_on='datadate') # PROFITABILITY
    factors = factors.merge(investment, how='inner', left_on='datadate', right_on='datadate') # INVESTMENT
    factors = factors.merge(defensive, how='inner', left_on='datadate', right_on='datadate') # DEFENSIVE
    factors = factors.merge(quality, how='inner', left_on='datadate', right_on='datadate') # QUALITY
    factors = factors.merge(returns_mkt, how='inner', left_on='datadate', right_index=True) # MARKET
